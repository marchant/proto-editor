...
exports.Editor = Component.specialize(/** @lends Main# */ {
    constructor: {
        value: function Main() {
          this.id = "edit";
    ...
    proj: {
      ...
      set: function(value) {
        value.identifier = "proj";
        this._proj = value;
      }
    },
    toggle: {
      value: function (opt, id) {

          if (opt == 1) {
              if (id.classList.contains("off")) {
                Array.prototype.forEach.call(document.querySelectorAll(".edit_rpane"),function(element) {
          ...
          if (opt == 1) {
              Array.prototype.forEach.call(document.querySelectorAll(".hedit_title"),function(element) {
      ...
      value: function(firstTime) {
        if(firstTime) {
          window.addEventListener("resize",function() {
                that.format(that.current_format);
                CSS.refresh();
                if (!that.clip) that.clip = document.getElementById("edit_live_clip");
                f = that.formats[that.current_format];
                if (that.onresize) that.onresize(f.width, f.height);
             //clearTimeout(resizeTimer);
             //var resizeTimer = setTimeout(AMK.doResize, 1000);
           },false);

           this.css.addEventLister
        }
    ...
    prepareForActivationEvents: {
      value:function() {
        this.cssPressComposer = new PressComposer();
        this.cssPressComposer.element = this.css;
        this.cssPressComposer.identifier = "CSSComposer";
        this.addComposer(this.cssPressComposer);
        this.cssPressComposer.addEventListener("press",this,false);

        this.proj.addEventListener("click",this,false);
      }
    },
    handleProjClick: {
      value: function(event) {
        this.toggle(0,this.proj);
      }
    },
    handleCSSComposerPress: {
      value: function(event) {
        this.toggle(0,event.target.element);
      }
    },
    handleEditInspectorOpenAction: {
      value: function(event) {
        this.toggle(1,this._editInspectorPanel);
      }
    },
    handleEditPaletteOpenAction: {
      value: function(event) {
        this.toggle(1,this._editPalettePanel);
      }
    },
    handleEditInspectorCloseAction: {
      value: function(event) {
        this.toggle(0,this._editInspectorPanel);
      }
    },
    handleEditPaletteCloseAction: {
      value: function(event) {
        this.toggle(0,this._editPalettePanel);
      }
    },
    draw: {
      value: function(firstTime) {
        if(firstTime) {
            ...
            CSS.build(this.css);
            //Getting the node programmatically
            //this.content = this.element.querySelector("#fullscreen");
            DOM.build(this.content,that.dom);
